...
def grid_values(grid):
    ...
    """
    assert len(grid) == 81
    new_grid = ['123456789' if x == '.' else x for x in grid]
...
def reduce_puzzle(values):#receives as input an unsolved puzzle and applies our two constraints repeatedly in an attempt to solve it.
    stalled = False
    while not stalled:
...
def search(values): #the last function to get the results(create a tree of possibilities and traverse it using DFS until it finds a solution)
